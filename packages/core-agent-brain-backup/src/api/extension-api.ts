...
import type {
    Pattern, Intent, InterventionStrategy, Learning, Command, Configuration, Logger,
...
export interface AgentBrainExtension {
     ...
     */
    patterns?(): Pattern[] | Promise<Pattern[]>;
...
export abstract class BaseExtension implements AgentBrainExtension {
    ...
    protected async loadPatternsFromFile(filePath: string): Promise<Pattern[]> {
    ...
    protected createPattern(config: Partial<Pattern>): Pattern {
        return {
            severity: 'warning',
            source: this?.name,
            ...config
